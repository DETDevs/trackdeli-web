      ...
      <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-5 animate-in fade-in zoom-in-95 duration-150">
        <div className="flex items-center justify-between border-b border-gray-100 pb-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">Credenciales de acceso inicial</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="space-y-4">
          <p className="text-xs text-gray-600">
            El usuario <span className="font-semibold text-gray-900">{user.name}</span> (
            <span className="font-mono text-gray-700">{user.email}</span>) con rol{' '}
            <span className="font-medium text-gray-900">{roleLabels[user.role] || user.role}</span>{' '}
            ha sido creado correctamente. Copia esta contraseña para transmitírsela:
          </p>
          ...
          <p className="text-[11px] text-gray-400 text-center">
            Podrás restablecer esta contraseña en cualquier momento desde esta plataforma.
          </p>
        </div>
